...
window.SeeFoodSegmenter = (function () {
    ...
    async function analyze(imgElement, overlayCanvas) {
        try {
            ...
            // Store detections for nutrition prediction
            if (window._lastYoloDetections !== undefined) {
                window._lastYoloDetections = yoloResult.detections || [];
            }
            return {
                mode: 'yolo',
...
window.SegmentationEditor = (function () {
    ...
    function init(pCanvas, sCanvas, cCanvas) {
        ...
        // Match sizes to segCanvas (reliable source of canvas dimensions)
        // photoCanvas may be an HTMLImageElement, use segCanvas size instead
        corrCanvas.width = segCanvas.width || 400;
        corrCanvas.height = segCanvas.height || 400;
    ...
    function updateMinimap() {
        if (!minimapCanvas || scale === 1) return;
        
        minimapCtx.clearRect(0, 0, minimapCanvas.width, minimapCanvas.height);
        // Gunakan segCanvas sebagai base minimap (photoCanvas bisa HTMLImageElement, tidak bisa drawImage langsung ke minimapCtx)
        if (segCanvas) minimapCtx.drawImage(segCanvas, 0, 0, minimapCanvas.width, minimapCanvas.height);
