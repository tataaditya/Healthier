...
# --- Anti CUDA OOM (RTX 3050 6GB) ---
# Batasi alokasi memori PyTorch (YOLO) agar tidak memonopoli VRAM, 
# menyisakan memori untuk PaddleOCR dan WebGL browser.
os.environ["PYTORCH_CUDA_ALLOC_CONF"] = "max_split_size_mb:128"

# Fix PaddlePaddle 3.x NotImplementedError (ConvertPirAttribute2RuntimeAttribute) dengan oneDNN
...
class PPOCREngine:
    ...
    def muat(self):
        ...
        ocr_kwargs = {
            ...
            "lang": "en",
            # 'use_gpu' dihapus di PaddleOCR 3.x — CPU adalah default jika tidak ada GPU
            "text_det_thresh": 0.3,
            "text_det_box_thresh": 0.5,
        }
        # ocr_version opsional (misal: "PP-OCRv4"). Kosongkan jika tidak perlu.
        if OCR_VERSION and OCR_VERSION.lower() not in ("pp-ocrv4", "default"):
            ocr_kwargs["ocr_version"] = OCR_VERSION
        self.ocr = PaddleOCR(**ocr_kwargs)
        print(f"   OCR Engine siap dalam {time.time()-t0:.1f} detik")
    ...
    def deteksi(self, img_input) -> dict:
        ...
        # PaddleOCR 3.x: ocr() masih bisa dipakai, tapi format output bisa berbeda
        ...
        if isinstance(res, dict):
            ...
            for line in res:
                ...
                # PaddleOCR 3.x bisa return dict {'text': ..., 'score': ...} (kalau masih array)
                # PaddleOCR 2.x return tuple (text, score)
                if isinstance(rec_result, dict):
                    teks = rec_result.get("text") or rec_result.get("rec_text", "")
                    conf = rec_result.get("score", rec_result.get("rec_score", 0.0))
                elif isinstance(rec_result, (list, tuple)) and len(rec_result) >= 2:
                    teks = rec_result[0]
                    conf = rec_result[1]
                else:
                    continue
...
class YOLOIsiPiringku:
    ...
    def get_instance(cls):
        if cls._instance is None:
            ...
            cls._instance = YOLO(str(model_path))
...
def buat_fastapi_app():
    ...
    # ── Food Nutrition Database Loading ──────────────────────────────

    import csv
    from difflib import SequenceMatcher

    FOOD_DB = {}  # Dict[str, {calories, protein, fat, carbohydrates}]
    FOOD_NAMES_LIST = []  # For fuzzy matching

    SERVING_REFERENCES = {
        "makanan_pokok": {"name": "Nasi/Karbo", "default_g": 150},
        "lauk_pauk": {"name": "Lauk Protein", "default_g": 75},
        "sayur": {"name": "Sayuran", "default_g": 100},
        "buah": {"name": "Buah-buahan", "default_g": 100},
    }

    def _load_nutrition_csv():
        """Load nutrition.csv (Indonesian food dataset, 1347 entries)."""
        path = os.path.join(os.path.dirname(__file__), "nutrition.csv")
        if not os.path.exists(path):
            print("⚠️ nutrition.csv not found, skipping.")
            return 0
        count = 0
        try:
                ...
                for row in reader:
                    try:
                        ...
                        FOOD_DB[name] = {
                            ...
                            "carbohydrates_g": float(row.get("carbohydrate", 0) or 0),
            ...
            print(f"⚠️ Error loading nutrition.csv: {e}")
        return count

    def _load_food_dataset():
        """Load FINAL FOOD DATASET (international, 5 group CSVs)."""
        dataset_dir = os.path.join(os.path.dirname(__file__), "FINAL FOOD DATASET")
        if not os.path.exists(dataset_dir):
            print("⚠️ FINAL FOOD DATASET folder not found, skipping.")
            return 0
        count = 0
        for i in range(1, 6):
            fname = f"FOOD-DATA-GROUP{i}.csv"
            ...
            try:
                with open(fpath, "r", encoding="utf-8") as f:
                    ...
                    for row in reader:
                        try:
                            name = (row.get("food") or "").strip().lower()
                            if not name or name in FOOD_DB:
                                continue  # Indonesian dataset has priority
                            FOOD_DB[name] = {
                                "calories": float(row.get("Caloric Value", 0) or 0),
                                "protein_g": float(row.get("Protein", 0) or 0),
                                "fat_g": float(row.get("Fat", 0) or 0),
                                "carbohydrates_g": float(row.get("Carbohydrates", 0) or 0),
                                "source": f"food_dataset_group{i}"
                            }
                            count += 1
                        except (ValueError, TypeError):
                            continue
    ...
    n1 = _load_nutrition_csv()
    n2 = _load_food_dataset()
    FOOD_NAMES_LIST.extend(sorted(FOOD_DB.keys()))
    print(f"✅ Food database loaded: {n1} Indonesian + {n2} international = {len(FOOD_DB)} total foods")

    def _fuzzy_match_food(query: str, threshold: float = 0.55):
        """Match a food label to the database using tiered matching.
        Returns: (food_name, nutrition_dict, confidence, match_type)
        """
        q = query.strip().lower()
        if not q:
            return None
        ...
        # Tier 1: Exact match
        if q in FOOD_DB:
            return (q, FOOD_DB[q], 1.0, "exact")

        # Tier 2: Normalized token match (e.g. "nasi putih" in "nasi putih goreng")
        for name in FOOD_NAMES_LIST:
            if q in name or name in q:
                return (name, FOOD_DB[name], 0.85, "substring")

        # Tier 3: Token-level match (any word matches)
        ...
        for name in FOOD_NAMES_LIST:
            ...
            if overlap > best_token_overlap and overlap >= 1:
                best_token_overlap = overlap
                best_token_match = name
        if best_token_match and best_token_overlap >= 1:
            conf = min(0.80, 0.55 + best_token_overlap * 0.15)
            return (best_token_match, FOOD_DB[best_token_match], conf, "token")

        # Tier 4: Fuzzy similarity (SequenceMatcher)
        best_score = 0.0
        best_name = None
        # Only check a sample for performance (first 500 names + any containing first word)
        first_word = q.split()[0] if q.split() else ""
        candidates = FOOD_NAMES_LIST[:500]
        if first_word:
            candidates = list(set(candidates + [n for n in FOOD_NAMES_LIST if first_word in n]))
        ...
        if best_name and best_score >= threshold:
            return (best_name, FOOD_DB[best_name], best_score, "fuzzy")

        return None

    # Fallback constants (when no match found)
    FALLBACK_NUTRITION = {
        "makanan_pokok": {"calories": 130, "protein_g": 2.7, "fat_g": 0.3, "carbohydrates_g": 28.0},
        "lauk_pauk": {"calories": 220, "protein_g": 20.0, "fat_g": 14.0, "carbohydrates_g": 3.0},
        "sayur": {"calories": 35, "protein_g": 2.0, "fat_g": 0.5, "carbohydrates_g": 6.0},
        "buah": {"calories": 50, "protein_g": 0.6, "fat_g": 0.2, "carbohydrates_g": 12.0},
    }
    ...
    async def health():
        ...
        try:
            model_path = Path(__file__).parent / "best.pt"
            if model_path.exists() or Path("best.pt").exists():
                yolo_status = "ready"
        except Exception:
            pass
    ...
    def ocr_endpoint(file: UploadFile = File(...)):
        ...
        try:
            with tempfile.NamedTemporaryFile(delete=False, suffix=suffix) as tmp:
                ...
                tmp_path = tmp.name
            hasil = jalankan_ocr(tmp_path)
            return _json_safe(hasil)
        finally:
            if tmp_path and os.path.exists(tmp_path):
                os.remove(tmp_path)

    @api.post("/api/isi-piringku")
    def isi_piringku_endpoint(file: UploadFile = File(...)):
        suffix = Path(file.filename or "upload.jpg").suffix or ".jpg"
        tmp_path = None
        try:
            with tempfile.NamedTemporaryFile(delete=False, suffix=suffix) as tmp:
    ...
    @api.post("/api/food-nutrition")
    async def food_nutrition_endpoint(request: Request):
        """Predict nutrition from detected food labels and proportions.
        
        Input: {
            "detected_labels": [
                {"label": "nasi putih", "category": "makanan_pokok", "proportion": 0.35},
                ...
            ],
            "plate_weight_g": 350  // optional
        }
        """
        try:
        ...
        plate_weight_g = payload.get("plate_weight_g", 350)
        
        if not detected:
            return {"status": "error", "message": "No detected labels provided"}
        ...
        for item in detected:
            ...
            proportion = float(item.get("proportion", 0.25))
            
            # Try to match food
            match = _fuzzy_match_food(label)
            
            serving_ref = SERVING_REFERENCES.get(category, {"default_g": 100})
            estimated_grams = serving_ref["default_g"] * proportion * 2  # Scale by proportion
            estimated_grams = max(30, min(500, estimated_grams))  # Clamp
            
            if match:
                food_name, nutrition, confidence, match_type = match
                scale = estimated_grams / 100.0
                cal = nutrition["calories"] * scale
                pro = nutrition["protein_g"] * scale
                carb = nutrition["carbohydrates_g"] * scale
                fat = nutrition["fat_g"] * scale
                ...
                fallback = FALLBACK_NUTRITION.get(category, FALLBACK_NUTRITION["lauk_pauk"])
                scale = estimated_grams / 100.0
                cal = fallback["calories"] * scale
                pro = fallback["protein_g"] * scale
                carb = fallback["carbohydrates_g"] * scale
                fat = fallback["fat_g"] * scale
                food_name = serving_ref.get("name", label)
                confidence = 0.3
                conf_level = "low"
                match_type = "fallback"
            ...
            components.append({
                ...
                "carbohydrates_g": round(carb, 1),
                "fat_g": round(fat, 1)
            })

        # Overall confidence level
        avg_confidence = (overall_confidence / match_count) if match_count > 0 else 0.3
    ...
    async def search_food(q: str = ""):
        ...
        results = []
        # Simple substring search prioritizing startswith, then contains
        for name in FOOD_NAMES_LIST:
            if query in name:
                # Calculate relevance score (lower is better, exactly match = 0, starts with = 1, contains = 2)
                if name == query:
                    score = 0
        ...
        # Limit to top 30 results to prevent massive payloads
        return {
            "status": "success",
            "results": results[:30],
